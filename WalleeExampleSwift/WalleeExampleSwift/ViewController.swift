...
class ViewController: UIViewController, WALPaymentFlowDelegate {
    ...
    func startPaymentWithBuilderSyntax() {
        ...
        let configuration = WALFlowConfiguration.make(with: builder, error: &error)
    ...
    func displayError(error: Error?) {
        ...
        let alert = UIAlertController(title: "Error Code \(error.localizedDescription)",
                message: error.localizedDescription,
                preferredStyle: .alert)
        let okay = UIAlertAction(title: "Okay", style: .default, handler: { (_ : UIAlertAction) -> Void in
        })
    ...
    func flowCoordinator(_ coordinator: WALFlowCoordinator!,
                         didSelectPaymentMethod paymentMethod: WALPaymentMethodConfiguration!) {}
